...
public class Find extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			...
			PreparedStatement pstmt = con.prepareStatement("select * from user where email=? and pwd=? and role=?");
...
//			}
			if(rs.next())
			{
				if(urole.equals("inventoryManager"))
				{
					response.sendRedirect("inventoryLogin.html");						
				}
				else if(urole.equals("accountant"))
				{
					response.sendRedirect("AccLogin.html");				
				}
				else 
				{
					response.sendRedirect("SalesLogin.html");				
				}
			}
			else
			{
